...
# for existing repos, it updates the "Valid-Until" field for 1 year
def fixReleaseHeader(filePath, suite):
  patternHash = 'Acquire-By-Hash: yes'
  newStrHash = 'Acquire-By-Hash: no'
  replaceDate = time.strftime("Date: %a, %d %b %Y %H:%M:%S UTC", \
      time.localtime())
  replaceValid = time.strftime("Valid-Until: %a, %d %b %Y %H:%M:%S UTC", \
      time.localtime(time.time() + 31557600.0))
  replaceSuite = suite
